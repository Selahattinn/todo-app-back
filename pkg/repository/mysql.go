...
import (
	...
	"github.com/Selahattinn/todo-app-back/pkg/repository/job"
...
func NewMySQLRepository(cfg *MySQLConfig) (*MySQLRepository, error) {
	...
	jobRepo, err := job.NewMySQLRepository(db)
	if err != nil {
		return nil, err
	}
...
// GetJobRepository returns the job repository
func (r *MySQLRepository) GetJobRepository() job.Repository {
	return r.jobRepo
}
