...
mysql-test-build:
	mysql -h localhost -P 3306 --protocol=tcp -u root -prootpw -e 'create database todos;'
	mysql -h localhost -P 3306 --protocol=tcp -u root -prootpw -e 'drop database todos;'
	./bin/todo-app-back
	mysql -h localhost -P 3306 --protocol=tcp -u root -prootpw -e 'use todos;insert into jobs (body) values("Test");insert into jobs (body) values("Test");select * from jobs'
